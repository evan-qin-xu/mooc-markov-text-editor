...
import com.zeroonejourney.spelling.AutoComplete;
import com.zeroonejourney.spelling.Dictionary;
import com.zeroonejourney.spelling.SpellingSuggest;
...
public class AutoSpellingTextArea extends StyledTextArea<Boolean> {
  ...
  // set up reflection for com.zeroonejourney.spelling suggest
  ...
  private final SpellingSuggest spellingSuggest;
  // track auto complete and com.zeroonejourney.spelling suggestion states
  ...
  public AutoSpellingTextArea(AutoComplete ac, SpellingSuggest ss, Dictionary dic) {
    ...
    this.spellingSuggest = ss;

    // dictionary used in org.evanxu.spelling suggestions
    this.dictionary = dic;
  ...
  /**
   * Checks org.evanxu.spelling of text in text area builds style spans
   *
  ...
  private void showSuggestions(String word, MouseEvent click) {
    List<String> suggestions = spellingSuggest.suggestions(word, NUM_SUGGESTIONS);
    ...
    item.setOnAction(
        new EventHandler<ActionEvent>() {
          ...
          public void handle(ActionEvent actionEvent) {
            ...
            ((Dictionary) autoComplete).addWord(word);
  ...
  private void showCompletions(String prefix) {
    ...
    if (!prefix.isEmpty()) {
      ...
      options = (autoComplete).predictCompletions(prefix, NUM_COMPLETIONS);
   ...
   *
   * @param word the word
   * @param flags the corresponding flags for each character
