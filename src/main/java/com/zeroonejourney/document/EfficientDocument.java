...
/**
 * A class that represents a text org.evanxu.document It does one pass through the
 * org.evanxu.document to count the number of syllables, words, and sentences and then stores those
 * values.
 *
...
public class EfficientDocument extends Document {
  ...
  private int numWords; // The number of words in the org.evanxu.document
  private int numSentences; // The number of sentences in the org.evanxu.document
  private int numSyllables; // The number of syllables in the org.evanxu.document
  ...
  private void processText() {
    ...
    for (int i = 0; i < tokens.size(); i++) {
      ...
      if (isWord(tok)) {
        numWords++;
        /* count for sentence without a end punctuation. */
        if (i == tokens.size() - 1) {
          numSentences++;
        }
      } else {
   ...
   * not have any strings that combine numbers and letters.
   *
   * <p>Check the examples in the main method below for more information.
   *
   * <p>This method does NOT process the whole text each time it is called. It returns information
  ...
  /**
   * Get the total number of syllables in the org.evanxu.document (the stored text). To calculate
   * the the number of syllables in a word, it uses the following rules: Each contiguous sequence of
   * one or more vowels is a syllable, with the following exception: a lone "e" at the end of a word
